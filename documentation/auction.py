...
def proccess_auction(message, user):
    ...
    if user.get("status") == "pending_offer":
        # Receive offer, check if it's in a valid range
        try:
            # Check if the user is the same as the highest offer
            if highest_offer > 0 and user.get("phone") == highest_offer_phone:
                send_whatsapp_message(
                    phone_number=message.get('from'),
                ...
                return
            # Check if the user wants to cancel the offer
            ...
            logger.info(f"Received offer: {message.get('text').get('body')}")
            offer = float(message.get("text").get("body"))
            if offer < MIN_BID:
                send_whatsapp_message(
                    phone_number=message.get('from'),
                    message=f"La oferta debe ser al menos de {format_as_money(MIN_BID)}. Por favor, ingrese un monto válido o cancele la oferta.",
                    buttons=[
                        {"id": "cancel_offer", "text": "Cancelar oferta"}
                    ]
                )
                return
            elif offer > MAX_BID:
                send_whatsapp_message(
                    phone_number=message.get('from'),
                    message=f"La oferta supera el límite máximo permitido de {format_as_money(MAX_BID)}. Por favor, ingrese un monto válido o cancele la oferta.",
                    buttons=[
                        {"id": "cancel_offer", "text": "Cancelar oferta"}
                    ]
                )
                return
            elif offer < highest_offer + MIN_BID_DIFFERENCE:
                send_whatsapp_message(
                    phone_number=message.get('from'),
                    message=f"La oferta debe ser superior a {format_as_money(highest_offer + MIN_BID_DIFFERENCE)}. Por favor, ingrese un monto válido o cancele la oferta.",
                    buttons=[
                        {"id": "cancel_offer", "text": "Cancelar oferta"}
                    ]
                )
                ...
                # Store amount as string to preserve decimal places
                user_table.update_item(
                    Key={'phone': message.get('from')},
                    UpdateExpression="SET #status = :status, #draft_bid = :bid",
                    ExpressionAttributeNames={'#status': 'status', '#draft_bid': 'draft_bid'},
                    ExpressionAttributeValues={':status': 'pending_offer_confirmation', ':bid': Decimal(str(offer))}
                )
                send_whatsapp_message(
                    phone_number=message.get('from'),
                    message=f"¿Desea confirmar su oferta de {format_as_money(offer)}?",
                    buttons=[
            ...
            send_whatsapp_message(
                phone_number=message.get('from'),
                message="Por favor, ingrese una oferta válida o cancele la oferta.",
                buttons=[
                    {"id": "cancel_offer", "text": "Cancelar oferta"}
                ]
            )
            return
    elif user.get("status") == "pending_offer_confirmation":
        ...
        if message.get("interactive"):
            if message.get("interactive").get("button_reply"):
                ...
                if button_id == "manage_notifications":
                    ...
                    send_whatsapp_message(
                        phone_number=message.get('from'),
                        message=f"Por favor, envíe su oferta en el siguiente formato: 1000000 (sin puntos, espacios o símbolos). Recuerde que la oferta debe ser superior o igual a {format_as_money(max(highest_offer + MIN_BID_DIFFERENCE, MIN_BID))}.",
                        buttons=[
                            {"id": "cancel_offer", "text": "Cancelar oferta"}
                        ]
                    )
                    # Update user status to pending_offer
                    user_table.update_item(
                        Key={'phone': message.get('from')},
                        UpdateExpression="SET #status = :status",
                        ExpressionAttributeNames={'#status': 'status'},
                        ExpressionAttributeValues={':status': 'pending_offer'}
                    )
                    return
...
def send_menu(message, user, highest_offer, highest_offer_phone):
    ...
    if user.get("phone") == highest_offer_phone:
        ...
        send_whatsapp_message(
            phone_number=message.get('from'),
            message=f"La subasta está en curso. La oferta más alta es de {format_as_money(highest_offer)}. ¿Desea realizar una oferta más alta o configurar próximas notificaciones?",
            buttons=[
        ...
        send_whatsapp_message(
            phone_number=message.get('from'),
            message="La subasta está en curso. Hasta el momento no se ha realizado ninguna oferta. ¿Desea realizar una oferta o configurar próximas notificaciones?",
            buttons=[
                {"id": "offer", "text": "Ofertar"},
                {"id": "manage_notifications", "text": "Notificaciones"}
            ]
